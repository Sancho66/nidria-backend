...
> **Superadmin plateforme — `GET /agencies` vs `GET /admin/agencies` (Groupe C UI, frontière figée)** : deux endpoints superadmin (gate `agency.create`), rôles disjoints, ne jamais confondre. **`GET /agencies`** = l'endpoint **LÉGER du switcher** (le sélecteur « Changer d'agence » du header : `id`/`name`/`slug` pour choisir où *entrer* = impersonation). **`GET /admin/agencies`** (`src/admin/`, additif, aucun endpoint existant touché) = l'endpoint **RICHE du tableau** « Gérer les agences » : paginé (`page`/`page_size`), recherche `name`/`slug`, tri `created_at|name|cases_count`, et par agence `{plan, seats_used (internes) / members_count (tous) / seats_limit, is_founding, status, trial_days_remaining, cases_count (vivants non-demo), logo_url, created_at}`. **`status` dérivé du modèle** (aucune colonne `status`/`suspended`) : `active` (converted_at, testé EN PREMIER) | `trial` (+jours) | `expired` (essai passé non converti) | `unknown` (ni essai ni conversion = anomalie hors-wizard, **jamais** rangée dans expired). **Perf** : 2 requêtes constantes (principale à sous-requêtes scalaires corrélées, donc `sort=cases_count` sans requête sup. + 1 `COUNT`), prouvé par un test qui compte les requêtes. La **suppression** (`DELETE /agencies/{id}`) part de ce tableau avec le token PROPRE du superadmin (hors impersonation) — sous impersonation le token porte l'admin cible sans `agency.create` → 403 (comportement voulu, on sort d'abord de l'impersonation). `GET /agencies` est **cantonné** au switcher, pas déprécié.
...
> **Contenu d'étape (Feature 2, V1 — note + pièces jointes agence, CRUD)** : contenu DESCENDANT que l'agence fournit sur une étape-template (distinct des réquisits qui DEMANDENT au client). `journey_template_step.content_note` (TEXT, posé via le PATCH étape existant) + nouvelle table `journey_step_attachment` (`step_id` FK CASCADE, `filename`, `storage_path`, `uploaded_by_agent_id` SET NULL, `position`). Le contenu vit sur le TEMPLATE → identique pour tous les dossiers du parcours. **Storage réutilisé** : le primitif `src/core/storage.*` (générique, path-based), PAS la table `document` (case-scopée) ; path `templates/{tid}/steps/{sid}/{aid}/{filename}`. **Cohérence storage↔DB** : à l'upload, storage PUIS insert — si l'insert échoue, le fichier est supprimé (zéro orphelin) ; au delete, row puis fichier (best-effort, un orphelin éventuel loggé, jamais une row pointant un fichier absent). CRUD agence `/journeys/{tid}/steps/{sid}/attachments` (GET/POST/GET download/DELETE, gate `journey.configure`) + `content_note`/`attachments[]` exposés dans `TemplateStepResponse` (batché, pas de N+1). **Helper `Content-Disposition` RFC 6266 mutualisé** : extrait du fix v0.7.1 vers `src/core/http.py` (`file_download_response`), réutilisé par documents ET les pièces jointes d'étape — le fix vaut des deux côtés par construction, jamais recopié. **V1 = agence uniquement** ; la lecture filtrée expat (toujours) / externe (étape-responsable seulement) = V2 (RGPD, tests d'évasion serveur-side). **Invariant écosystème pays = zéro** (les 7 points intouchés ; nouveau contenu d'étape, ne touche pas `client_case`). Migration additive `a1d4f8c2e6b9` (1 colonne nullable + 1 table).

> **Contenu d'étape (Feature 2, V2 — lecture filtrée expat/externe, le morceau RGPD)** : le `content_note` + les `attachments` (posés sur le TEMPLATE en V1) deviennent lisibles côté faces. **L'invariant clé** : le contenu vit sur le template, mais **le DROIT vit sur le dossier** — `case_step_progress.responsible_agent_id`, une colonne d'INSTANCE. **EXPAT** : voit toujours le contenu sur son propre dossier. **EXTERNE** : voit le contenu d'une étape **uniquement s'il est responsable de cette étape sur CE dossier** (`responsible_agent_id == external.id`) — sinon `content_note=None`, `attachments=[]`, **filtré serveur-side** (jamais sur le fil), pas masqué front. Le `StepProgressResponse` interne porte le contenu (batché dans `timeline_for_case`, zéro N+1) ; les 3 faces projettent (agence/expat toujours, externe gaté par `_external_sees_content`, le verrou portant le commentaire garde-fou « si un legacy `external_contact` gagne un login un jour → élargir ici »). **2 downloads gatés** (`/expat/.../steps/{progress_id}/attachments/{aid}/download` toujours ; `/external/...` gaté `responsible_agent_id == external.id` → 404 sinon, jamais un octet servi) ; double barrière : l'attachment doit appartenir au `template_step_id` du progress (un progress_id arbitraire ne sert pas une pièce d'une autre étape). `file_download_response` réutilisé (3ᵉ fois). **Tests d'évasion HTTP** (`test_step_content_read.py`) : expat toujours / externe responsable 200 / externe non-responsable contenu absent **et** download 404 / **LE CROISEMENT X vs Z** (même externe, même étape-template, deux dossiers → 200 sur celui dont il est responsable, 404 sur l'autre — preuve runtime que c'est la colonne d'instance qui filtre, pas le template) / expat d'un autre dossier 404. **Écosystème pays = zéro** sur les 5 points (cases_repository, filter_builder, dashboard, views_schema, case_export) ; seuls `expat_schema` + `external_schema` gagnent le contenu (attendu). Pas de migration (additif schéma). V3 : éditeur agence front (bouton + textarea + upload) · V4 : affichage lecture-seule client/externe + i18n.

> **« Action validée par » (refonte `completion_mode`, V1 backend)** : nouvelle notion par étape, symétrique au responsable (type au TEMPLATE, personne au DOSSIER), qui absorbe l'ex-« Mode de complétion ». `StepValidatorType = none|expat|agent|external` (`none`=auto, `expat`=client, `agent`=agence, `external`=prestataire désigné). **D1** : `validated_by_type`/`validated_by_agent_id` copiés du template (`default_validated_by_*`) et **FIGÉS** sur `case_step_progress` à l'assignation — éditer un parcours ne change pas rétroactivement les dossiers en cours. **D2** : CHECK `validated_by_type_matches_fk` souple — `agent`⟹agent_id optionnel (NULL=agence en général), `external`⟹agent_id NOT NULL (provider, un Agent `is_external`, pas un `external_contact` : un validateur se connecte et clique). **Migration `b2e9d6a4c8f1`** additive/réversible : nullable→backfill déterministe depuis `completion_mode` (`auto→none`, `agency_validation→agent`)→NOT NULL+`server_default 'agent'`→CHECK ; **`completion_mode` CONSERVÉ** (filet rollback, jamais touché → downgrade restaure 100 % ; DROP = vague ultérieure). Idempotente, couvre toutes les lignes (`test_migration_step_validator` : backfill + idempotence + réversibilité). **Moteur** (`recompute_active`) lit le `validated_by_type` de l'INSTANCE : `none`→auto-DONE (= ex-`auto`, inchangé), `agent`→mail+clôture manuelle (= ex-`agency_validation`, inchangé), `expat`/`external`→jamais d'auto-complétion, clôture par l'action dédiée. **Le PATCH done agent reste toujours autorisé** (sécurité opé). **Endpoints** : `PUT /cases/{id}/steps/{pid}/validator` (agence, case.edit, désigne au dossier) ; `POST /expat|external/cases/{id}/steps/{pid}/validate` (client / provider désigné — évasion 404 serveur-side si non-désigné, nouvelle perm `external.step.validate`). Faces : `validated_by_*` sur la timeline agence, `can_validate` (calculé serveur-side) sur expat/externe. **Écosystème pays = zéro** (5 points durs intouchés ; orthogonal). Note de conception : `docs/step-validator.md`. V2+ front : sélecteur validateur (remplace le toggle complétion) + boutons « valider » client/externe + renommage labels + i18n ; vague backend ultérieure : DROP `completion_mode`.

> **Refonte adresse/pays — V2 : type de champ `address` (sous-objet structuré)** : `CustomFieldType += ADDRESS` — un sous-objet `{street?, city?, postal_code?, country?}` (sous-champs string optionnels, longueurs max 255/100/20). Le sous-champ `country` **réutilise `_coerce_country` (la fonction V1, pas une recopie du pattern)** → si la règle pays change, elle change aux deux endroits par construction. Valeur « fournie » = sous-objet non-vide ; `_is_empty` devient **dict-aware** (`{}` ou tous-null → vide → clé effacée → pending ; raffinement « tous sous-champs requis » différé). Stockage `case_person.custom_fields` JSONB (plan personne), jamais `client_case`. **2 fichiers de prod** (`enums.py` + `custom_fields_validation.py`) — `requirements_eval` non touché (l'effacement au write rend la cohérence is_provided gratuite). **Invariant écosystème pays = zéro ligne** (7 points intouchés, vérifié au diff). Pas de migration. V3 : renderers front (CountrySelect + sous-formulaire adresse) · V4 : 3 colonnes de base.

> **Refonte adresse/pays — V1 : type de champ `country` (coexistence)** : `CustomFieldType` gagne **`COUNTRY`** — un sélecteur pays ISO-2 réutilisable (le « moule »). Valeur dans `case_person.custom_fields` JSONB (plan personne), **JAMAIS** sur `client_case`. `_coerce_one` gagne une branche `country` (pattern `^[A-Z]{2}$`, **la même règle** que `CaseUpdateRequest.origin_country` → un champ-pays custom valide à l'identique des colonnes canoniques). **Coexistence, pas remplacement** : `origin_country`/`dest_country` restent deux colonnes câblées, lues à l'identique par les 7 points de l'écosystème pays (dashboard/filtres/export/repository/schemas) — **zéro ligne touchée** (vérifié au diff). Le type est le moule pour les NOUVEAUX champs-pays (nationalité conjoint, pays de naissance…) ; les 2 canoniques ne sont pas dissoutes. Pas de migration (valeur d'enum + branche de validation). V2 : type `address` (sous-objet, country réutilise V1) · V3 : renderers front · V4 : 3 colonnes de base (birth_name/profession/employer).

> **Canevas visuel de parcours — MVP-1 (backend layout)** : stockage des positions de nœuds pour l'éditeur canevas (vue alternative React Flow de l'éditeur liste, même modèle). Colonne `journey_template.canvas_layout` **JSONB nullable** — blob `{ "<step_id>": {"x", "y"} }`, **pure présentation** (ne touche JAMAIS la logique du parcours, jetable). `PUT /journeys/{tid}/canvas-layout` (gate `journey.configure`) **remplace** le blob entier ; les step_ids **étrangers/périmés sont droppés serveur-side** (le blob ne pourrit pas — seuls les steps courants du template survivent). Exposé `canvas_layout: dict | None` dans `JourneyTemplateDetailResponse` (`None` = jamais ouvert en canevas → le front auto-layout dagre). **C'est TOUT côté backend** : nœuds/arêtes/édition réutilisent les endpoints existants (`steps`, `prerequisites`). Migration additive `f3c7a9e1b5d8` (1 colonne nullable). Le badge « N réquisits » reste hors MVP (demanderait un compte non présent dans `TemplateStepResponse`). MVP-2/3/4 (front) : canevas read-only lazy → édition d'arêtes (`setPrerequisites`, cycle 422→toast) → auto-save positions.

> **Réquisit case-level — fulfillment client (chantier sections, VAGUE C2 — écriture bornée)** : le CLIENT (espace expat) peut remplir un réquisit case-level → écrit une **colonne de `client_case`** (pays/adresse). Endpoint **séparé** `PUT /expat/cases/{case_id}/case-requirements/{step_case_requirement_id}`, body **`{value}` UNIQUEMENT** (jamais un nom de colonne). **4 bornes serveur, aucune confiance au payload** (calque du fulfillment personne vague 2) : (a) `get_case_for_expat` → 404 (son dossier, ownership) ; (b) `get_case_requirement_in_case` — jointure `step_case_requirement.step_id == case_step_progress.template_step_id ∧ case_id` → 404 (déclaration d'un autre dossier/parcours invisible) ; (c) progress `IN_PROGRESS` → 409 (étape à venir/terminée = lecture seule) ; (d) **LA BORNE CRITIQUE** : `column = creq.case_field` (vient de la **déclaration serveur**, JAMAIS du payload) + garde `∈ COLLECTABLE_CASE_FIELDS` ; `setattr` d'**UNE** colonne, la déclarée — `status`/`owner_agent_id`/`agency_id`/toute colonne non déclarée **inatteignables** (les clés bricolées du payload sont ignorées par `RequirementValueRequest`). Valeur validée via `CaseUpdateRequest.model_validate({column: value})` (mêmes patterns/longueurs que l'agence → 422 si invalide ; null efface → pending). Après écriture : `snapshot`→`setattr`→`recompute_active` (folde les case-reqs depuis C1 → auto-complétion / mail ready-to-validate)→`commit`→`send_pending` **best-effort** (un mail qui throw ne rollback jamais l'écriture). Le projection expat passe désormais `target` (le front route vers le bon endpoint). **Non-régression** : zéro modif hors `expat/*` (3 fichiers) ; écosystème pays (tri/filtre/KPI/vues/export) intouché ; fulfillment personne vague 2 inchangé (endpoint distinct). Pas de migration (table C1). **Test d'évasion** (le test qui vaut tout) : payload bricolé visant `owner_agent_id`/`status`/`origin_country` quand la déclaration = `dest_city` → **relecture en base après commit** : seule `dest_city` bouge, les autres colonnes intactes. La borne tient en RÉEL, pas en intention.

> **Réquisit case-level — l'unification (chantier sections, VAGUE C1 — lecture/projection/complétion)** : un parcours peut EXIGER un champ-dossier (pays/adresse) comme réquisit d'étape, **sans casser l'invariant person-keyed des réquisits ni l'écosystème pays**. Décision figée **option B + feuille de C** : table de DÉCLARATION `step_case_requirement` (`step_id` FK CASCADE, `case_field` ∈ `COLLECTABLE_CASE_FIELDS`, `position`, `UNIQUE(step_id, case_field)`) — **SANS `person_id`, SANS `scope`, SANS table concrète matérialisée**. Insight : un réquisit de type CHAMP n'a rien à matérialiser (valeur dérivée live du backing store, jamais stockée) ; un champ-dossier a une valeur case-wide unique, pas de personne, pas de composition à figer → déclaration + évaluation live de `client_case`. `case_step_requirement` reste **strictement person-keyed** (pas de `person_id` nullable, pas de discriminant `target` dans la table centrale — l'option A rejetée). **RÉSOLVEUR FACTORISÉ** (`requirements_eval`) : `resolve_provided`/`resolve_value(plane, ref, person=, case=)` tranchent la décision « fourni ?/valeur ? » UNE fois, en dispatchant **à la feuille** (`_person_field_value` sur case_person vs `_case_field_value` sur client_case) ; `is_provided`/`current_value` deviennent des **façades** plan-personne (comportement personne **prouvé inchangé** par un test dédié). Projection : `timeline_for_case` folde les case-reqs (déclarations lues live) dans `requirements[]` (même `RequirementStateResponse`, `target="case"`, `person_id=None`, `scope=None`, `kind="case_field"`) + `all_requirements_met` ; les 3 faces — agence/expat voient la `value`, **l'externe la strippe déjà uniformément** (RGPD, `person_id=None` ne crashe pas). Complétion : `_step_met` exige (réquisits-personne ∧ case-reqs) fournis → un case-req non-fourni **bloque** l'auto-complétion / le mail ready-to-validate. `update_case` (PATCH dossier agence) déclenche désormais `recompute_active` (l'agence remplit le champ-dossier → l'étape `auto` se complète). CRUD `/journeys/{tid}/steps/{sid}/case-requirements` (4 endpoints, gate `journey.configure`, calque `step_requirement` : hors whitelist → 422, dup → 409, scoping 404, reorder dense). **PAS d'écriture client** (c'est C2). **Non-régression pays** : écosystème READ (tri/filtre/KPI/vues/export) **zéro modif** ; le pays ne bouge jamais de `client_case`. `RequirementStateResponse`/`Expat`/`External` : `scope` nullable + `target` (défaut `"person"` → front actuel inchangé). Migration additive `e2b5c8d4f1a7`. **Vague C2** : fulfillment client `PUT /expat/cases/{id}/case-requirements/{id}` (la colonne vient de la déclaration, jamais du payload).
